...
<footer class="border-t border-sand-800 bg-sand-900 px-6 py-12 lg:px-8">
	<div class="mx-auto flex max-w-6xl flex-col items-center gap-6 text-center sm:flex-row sm:justify-between sm:text-left">
		...
		<nav class="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
			{#each links as link (link.href)}
				<a href={link.href} class="text-sm text-sand-400 transition-colors hover:text-sand-50">
					{link.label}
				</a>
			{/each}
		</nav>
